...
def main():
    ...
    for _ in range(P):
        ...
        if _ == 0:
            with open('Data/ip_address_file.txt', 'w') as file:
                file.write(hostname)
        server.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
        server.bind((hostname, args.base_port + _ + 1))
        with open(f'Data/server_ready_{_ + 1}.txt', 'w') as f:
    ...
    directory = 'Data/N{}_M{}_C{}_P{}_B{}/Server'.format(N, M, C, P, B)
    if os.path.exists(directory):
        shutil.rmtree(directory)
    os.makedirs(directory, exist_ok=True)
    ...
    for n in range(num_loops):
        if n > 0:
            with open(f'Data/server_ready_loop_{n}.txt', 'w') as f:
                f.write('Server is ready')
        timing = time.time()
    ...
    np.save('Data/N{}_M{}_C{}_P{}_B{}/neg_log_transfer.npy'.format(N, M, C, P, B), neg_log_p)
