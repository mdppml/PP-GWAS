...
def make_matrix2(N, columns, seed):
    ...
    while len(valid_columns) < columns:
        ...
        while X_chunk is None or X_chunk.shape[0] < N:
            snpdata = snp_gen(fst=.1, dfr=.25, iid_count=N+x, sid_count=chunk_size, maf_low=.05, seed=seed, chr_count=chr_count)
            X_chunk = snpdata.val
            x = x + 5000
            del snpdata
        X_chunk = X_chunk[:N, :]
...
def main():
    ...
    directory = {}
    for p in range(P):
        directory[p] = "../test_site/Data/N{}_M{}_C{}_P{}_B{}/Party_{}".format(N, M, C,P, B, p + 1)
